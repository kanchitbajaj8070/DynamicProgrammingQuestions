...
Harry Potter has n mixtures in front of him, arranged in a row. Each mixture has one of 100
...
that stand next to each other and mix them together, and put the resulting mixture in their place.
...
The first scenario is a much better way to proceed.*/
    ...
    public static void main(String[] args) {
        ...
        for(int[]e:dp)
            Arrays.fill(e,-1);
        System.out.println(   MixturesSolveByDP(a,0,a.length-1,dp));
    }
public static int cumulativeSum( int [] mixture , int i, int j)
{   int sum=0;
    for (int k = i; k <=j ; k++) {
        sum+=mixture[k];
    }
    ...
    private static int MixturesSolveByDP(int []mixture,int s, int e,int[][]dp) {
        if (s >= e)
            return 0;
        if (dp[s][e] != -1)
            return dp[s][e];
